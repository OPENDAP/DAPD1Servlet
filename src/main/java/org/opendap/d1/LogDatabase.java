...
public class LogDatabase {
	 ...
	 * 
	 */
	public LogDatabase(String dbName, String nodeId) throws SQLException, ClassNotFoundException {
		this.nodeId = nodeId;
		this.dbName = dbName;
	 ...
	 * @throws SQLException
	 */
	public void addEntry(Statement stmt, String PID, String ipAddress, String userAgent, String event) throws SQLException {
	...
	public Log getMatchingLogEntries(String where, int start, int count) throws SQLException, DAPDatabaseException {
		...
		try {
			...
			while (rs.next()) {
				...
				// TODO Could use rs.absolute(start+1) before loop; see docs; absolute() uses 1s indexing
				++i;
				if (i <= start)
					continue;
				if (i > start + count)
